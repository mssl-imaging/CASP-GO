...
c2Wrapper::c2Wrapper(string strParam)
{
    //introduction
    cout << "*************************************************************************" << endl;
    cout << "*                                                                       *" << endl;
    cout << "*  Welcome to use the CASP-GO auto-DTM system v2.0                      *" << endl;
    cout << "*  >> Carbonite2 processing                                             *" << endl;
    ...
    cout << "*  (for technical information)                                          *" << endl;
    cout << "*  m.chernetskiy{at}ucl.ac.uk                                           *" << endl;
    cout << "*  (for project information)                                            *" << endl;
...
void c2Wrapper::prepareProjectParam(){
    ...
    m_strRPCListFile = (string)tl["strRPCListFile"];
...
bool c2Wrapper::validateProjParam(){
    bool bRes = true;
    QDir qdir;
    ...
    if (!qdir.exists(m_strRPCListFile.c_str())){
        cerr << "ERROR (1): RPC list file does not exist." << endl;
        bRes = false;
    }
...
bool c2Wrapper::validateProjInputs(){
    bool bRes = true;
    ...
    cout << "Info: CASP-GO is loading stereo image PATHs from the stereo list file provided." << endl;
    while(sfStereoList)
    {
    ...
    sfStereoList.close();
    cout << "Info: a total number of " << nNum << " stereo image PATHs have been loaded to buf." << endl;
    if(nNum%2 != 0){
    ...
    nNum = 0;
    ifstream sfRPCList(m_strRPCListFile.c_str());
    cout << "Info: CASP-GO is loading RPC PATHs from the RPC list file provided." << endl;
    while(sfRPCList)
    {
        if( getline( sfRPCList, buf ) )
        {
            nNum ++;
            m_strvRPCList.push_back(buf);
        }
    }
    sfRPCList.close();
    cout << "Info: a total number of " << nNum << " RPC PATHs have been loaded to buf." << endl;
    ...
    if(m_strvStereoList.size()!= m_strvRPCList.size()){
        cerr << "ERROR (1): Total number of the images does not equal to the total number of RPC entries." << endl;
        bRes = false;
        exit(1);
    }
...
void c2Wrapper::doBatchProcessing(){
    int n = m_strvStereoList.size();
    int nCount = 1;
    for (nCount = 1; nCount <= n/2; nCount++){
        cout << endl;
        cout << endl;
        cout << "Info: CASP-GO is making a copy of no. " << nCount << " of the " << n/2 << " stereo pairs." << endl;
        dataPrep(nCount);

        if (checkDataPrep(nCount)){
            cout << endl;
            cout << endl;
...
void c2Wrapper::dataPrep(int nNum){
    int m_nNum = nNum;
    int m_nNumAll = m_nNum - 1;
    ...
    string m_strLeftImageID = m_strvLeftIDList[m_nNumAll];
    string m_strLeftImageFile = m_strInputDir + m_strLeftImageID + ".tif";
    string m_strRightImageID = m_strvRightIDList[m_nNumAll];
    string m_strRightImageFile = m_strInputDir + m_strRightImageID + ".tif";

    ostringstream strCmdLeft;
    strCmdLeft << "cp " << m_strLeftImageURL << " " << m_strLeftImageFile;
    system(strCmdLeft.str().c_str());

    ostringstream strCmdRight;
    strCmdRight << "cp " << m_strRightImageURL << " " << m_strRightImageFile;
    system(strCmdRight.str().c_str());
}

bool c2Wrapper::checkDataPrep(int nNum){
    bool bRes = true;

    cout << "Info: CASP-GO is checking the stereo pair just copied." << endl;
    ...
    if (!qdir.exists(m_strLeftImageFile.c_str())){
        cerr << "ERROR (2): the hard-copy of number " << m_nNum+1 << " of stereo left image (ID: "
             << m_strLeftImageID << ") has failed. Skipping this stereo pair."
             << " Please check the disk permissions." << endl;
        bRes = false;
    }
    ...
    if (!qdir.exists(m_strRightImageFile.c_str())){
        cerr << "ERROR (2): the hard-copy of number " << m_nNum+1 << " of stereo right image (ID: "
...
void c2Wrapper::dataConversion(int nNum){
    int m_nNum = nNum;
    int m_nNumAll = m_nNum - 1;
    int m_nLeftRPC = 2*m_nNum - 2;
    int m_nRightRPC = 2*m_nNum - 1;
    string m_strLeftImageID = m_strvLeftIDList[m_nNumAll];
    string m_strLeftRPCFile = m_strvRPCList[m_nLeftRPC];
    string m_strLeftImageFile = m_strInputDir + m_strLeftImageID + ".tif";
    string m_strLeftOut = m_strInputDir + m_strLeftImageID + "_RPC.tif";

    string m_strRightImageID = m_strvRightIDList[m_nNumAll];
    string m_strRightRPCFile = m_strvRPCList[m_nRightRPC];
    string m_strRightImageFile = m_strInputDir + m_strRightImageID + ".tif";
    string m_strRightOut = m_strInputDir + m_strRightImageID + "_RPC.tif";


    cout << "Info: CASP-GO is attaching the RPC information to the TIFF image." << endl;

    string m_strConversionSW = m_strWorkingDir + "UCL/rpc-meta.py";
    ostringstream strCmdConvert1;
    strCmdConvert1 << "python " << m_strConversionSW << " --c2_tif " << m_strLeftImageFile
                  << " --c2_rpc " << m_strLeftRPCFile << " --c2_out " << m_strLeftOut;
    system(strCmdConvert1.str().c_str());

    ostringstream strCmdConvert2;
    strCmdConvert2 << "python " << m_strConversionSW << " --c2_tif " << m_strRightImageFile
                  << " --c2_rpc " << m_strRightRPCFile << " --c2_out " << m_strRightOut;
    system(strCmdConvert2.str().c_str());

    QDir qdir;
    if (!qdir.exists(m_strLeftOut.c_str())){
        cerr << "ERROR (3): the pre-process of number " << m_nNumAll+1 << " of stereo left image (ID: "
             << m_strLeftImageID << ") has failed. Skipping this stereo pair."
             << " Please check the data and RPC source manually." << endl;
    }

    if (!qdir.exists(m_strRightOut.c_str())){
        cerr << "ERROR (3): the pre-process of number " << m_nNumAll+1 << " of stereo right image (ID: "
             << m_strRightImageID << ") has failed. Skipping this stereo pair."
             << " Please check the data and RPC source manually." << endl;
    }
...
bool c2Wrapper::checkDataConversion(int nNum){
    bool bRes = true;
...
void c2Wrapper::dataProcessP1(int nNum){
    ...
    string m_strLeftImageID = m_strvLeftIDList[m_nNum];
    string m_strLeftImageFile = m_strInputDir + m_strLeftImageID + "_RPC.tif";
    string m_strRightImageID = m_strvRightIDList[m_nNum];
    string m_strRightImageFile = m_strInputDir + m_strRightImageID + "_RPC.tif";
...
bool c2Wrapper::checkDataProcessP1(int nNum){
    bool bRes = true;
    ...
    string m_strLeftImageFile = m_strOutputDir + m_strResID + "/ba-" + m_strLeftImageID + "_RPC.adjust";
    string m_strRightImageFile = m_strOutputDir + m_strResID + "/ba-" + m_strRightImageID + "_RPC.adjust";

    QDir qdir;
    if (!qdir.exists(m_strLeftImageFile.c_str())){
        cerr << "ERROR (3): the process (Stage-1) of number " << m_nNum+1 << " of stereo left image (ID: "
             << m_strLeftImageID << ") has failed. Skipping this stereo pair."
             << " Please check if the RPC data has been successfully attached to the image." << endl;
        bRes = false;
    }
...
void c2Wrapper::dataProcessP2(int nNum){
    ...
    strCmdP21 << m_strP2SW << " -t rpc -s " << m_strAspParamFile << " "
    ...
    if (m_nMode == 1){
        ...
        strCmdP22 << m_strP2SW << " -t rpc -s " << m_strAspParamFile << " "
                  << m_strLeftImageFile << " " << m_strRightImageFile << " "
                  << m_strResP2 << " --bundle-adjust-prefix " << m_strResBA
        ...
        strCmdP23 << m_strP2SW << " -t rpc -s " << m_strAspParamFile << " "
                  << m_strLeftImageFile << " " << m_strRightImageFile << " "
                  << m_strResP2 << " --bundle-adjust-prefix " << m_strResBA
                  << " --threads " << m_nThreads << " --corr-timeout 600";
        ...
        strCmdP24 << m_strP2SW << " -t rpc -s " << m_strAspParamFile << " "
                  << m_strLeftImageFile << " " << m_strRightImageFile << " "
                  << m_strResP2 << " --bundle-adjust-prefix " << m_strResBA
                  << " --threads " << m_nThreads << " --corr-timeout 600";
        ...
        string m_strResORI3tmp = m_strResHB2 + "-DRG.tif";
        ...
        strCmdHB1_1 << m_strProcHB1_1 << " -t rpc -s " << m_strAspParamFile << " "
                    << m_strLeftImageFile << " " << m_strRightImageFile << " "
                    << m_strResHB1 << " --enable-fill-holes --fill-holes-max-size 2500 --threads " << m_nThreads << " --corr-timeout 900";
        system(strCmdHB1_1.str().c_str());

        ostringstream strCmdHB1_2;
        string m_strProcHB1_2 = m_strWorkingDir + "ASP/bin/point2dem";
        strCmdHB1_2 << m_strProcHB1_2 << " " << m_strResPC1
                    << " --datum WGS_1984 --dem-hole-fill-len 150 --median-filter-params 25 25.5"
                    << " --threads " << m_nThreads;
        system(strCmdHB1_2.str().c_str());

        ostringstream strCmdHB2_1;
        string m_strProcHB2_1 = m_strWorkingDir + "ASP/bin/parallel_stereo";
        strCmdHB2_1 << m_strProcHB2_1 << " -t rpc -s " << m_strAspParamFile << " "
                    << m_strLeftImageFile << " " << m_strRightImageFile << " "
                    << m_strResHB2 << " --stereo-algorithm 2 --corr-kernel 9 9 --corr-tile-size 512 --cost-mode 4"
                    << " --median-filter-size 15 --texture-smooth-size 13 --texture-smooth-scale 0.15 --sgm-collar-size 384"
                    << " --corr-blob-filter 11 --enable-fill-holes --fill-holes-max-size 2500";
        system(strCmdHB2_1.str().c_str());

        ostringstream strCmdHB2_2;
        string m_strProcHB2_2 = m_strWorkingDir + "ASP/bin/point2dem";
        strCmdHB2_2 << m_strProcHB2_2 << " " << m_strResPC2 << " --orthoimage " << m_strL1
                    << " --datum WGS_1984 --dem-hole-fill-len 250 --orthoimage-hole-fill-len 150 --median-filter-params 25 25.5"
                    << " --threads " << m_nThreads;
        ...
        strCmdHB3 << m_strProcHB3 << " " << m_strResDEM2 << " " << m_strResDEM1
                  << " -o " << m_strResHB1 << " --max-displacement 3000 --datum WGS_1984"
                  << " --save-transformed-source-points --num-iterations 2000 --max-num-reference-points 500000"
        ...
        strCmdHB4 << m_strProcHB4 << " " << m_strResPC1_trans << " --dem-hole-fill-len 750 --threads " << m_nThreads;
        ...
        strCmdHB5 << m_strProcHB5 << " " << m_strResDEM2 << " " << m_strResDEM1_trans << " -o "
                  << m_strResP2 << " --priority-blending-length 250 --threads " << m_nThreads;
...
void c2Wrapper::initialFeatureMatching(int nNum){
...
void c2Wrapper::densification(int nNum){
...
bool c2Wrapper::checkDataProcessP2(int nNum){
    bool bRes = true;
    ...
    if (m_nMode != 3){
        ...
        cout << " Info: the process (Stage-2) of number " << m_nNum+1 << " of stereo pair (ID: "
             << m_strResID << ") is skipped in hybrid mode." << endl;
    }
...
void c2Wrapper::dataProcessP3(int nNum){
    ...
    if (m_nMode == 2){
        ...
        strCmdP38 << m_strP3SW << " -t rpc -s " << m_strAspParamFile << " "
                  << m_strLeftImageFile << " " << m_strRightImageFile << " "
        ...
        strCmdP39 << m_strP3SW << " " << m_strResP2PC << " --orthoimage " << m_strResP2L
                  << " --datum WGS_1984 --dem-hole-fill-len 250 --orthoimage-hole-fill-len 250 --median-filter-params 25 25.5"
                  << " --threads " << m_nThreads;
...
bool c2Wrapper::checkDataProcessP3(int nNum){
    bool bRes = true;
...
void c2Wrapper::dataProcessP4(int nNum){
    ...
    strCmdP41 << "mv " << m_strResP3DEM << " " << m_strResP4DTM;
    system(strCmdP41.str().c_str());

    ostringstream strCmdP43;
    strCmdP43 << "mv " << m_strResP3DRG << " " << m_strResP4ORI32;
...
void c2Wrapper::saveMetaData(int nNum){
    ...
    if (sfMeta.is_open()){
        ...
        sfMeta << "      OperatingSystem = TBD" << endl;
        ...
        sfMeta << "      DTMResolution = TBD - NO PROJECTION" << endl;
        sfMeta << "      ORIResolution = TBD - NO PROJECTION" << endl;
        sfMeta << "      Unit = Degree" << endl;
        sfMeta << "      NodataValue = -3.4028234663852886e+38" << endl;
        sfMeta << "      Reference datum = WGS_1984" << endl;
        sfMeta << "    End_Object" << endl;
        sfMeta << "  End_Object" << endl;
        sfMeta << endl;
